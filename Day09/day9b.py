class History:
    ...
    def getForwardExtrapolatedValue(self):
    ...
    def getBackwardExtrapolatedValue(self):
        differences = History.getDifferencesInSequenceWithDifferencesInDifferences(self.numbers)
        total_difference_for_extrapolated_value = differences[-1][0]
        differences.pop(-1)
        for difference_sequence in reversed(differences):
            total_difference_for_extrapolated_value = difference_sequence[0] - total_difference_for_extrapolated_value
        return self.numbers[0]-total_difference_for_extrapolated_value
...
for history in histories:
    total+= history.getBackwardExtrapolatedValue()
print(total)